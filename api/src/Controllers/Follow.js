...
class Follow {
  ...
  followAction = async (req, res) => {
    ...
    try {
      let userFwer;
      try {
        userFwer = await User.findOne({
          //busco a user1. El user seguidor
          where: {
      ...
      let userFwed;
      try {
        userFwed = await User.findOne({
          //busco a user2. El user seguido
          where: {
      ...
      const userFollower = await Followed.findOne({
      ...
      if (userFollower) {
        return res.status(400).json({ msgE: "You already follow this user" });
      }

      const userFollowerCreate = await Follower.create({
      ...
      const followeded = await this.followed(
        userFwer.idUser,
        userFwed.userName,
        userFwed.idUser
      );
      if (follower)
        return res.status(200).json({ msg: "User followed correctly" });
      return res
        .status(418)
        .json({ msgE: "There was an error trying to follow the user" });
    } catch (error) {
      return res.status(404).json({ msgE: "The follow action failed" });
    }
  };

  unFollowAction = async (req, res) => {
    //user1 -/->  user2
    const { token, unfollowedUsername } = req.body; //llega el token de user1
    const tokenDecode = jwt.decode(token);
    try {
      ...
      if (!findFollower)
        return res
          .status(404)
          .json({ msgE: "You are not following this user" }); //si no lo encuentra no lo sigue
      const followedDelete = await this.unFollowed(
        userFwed.userName,
        userFwer.idUser
      );
  ...
  followed = async (idFollower, userNameFollowed, idFollowed) => {
    ...
    const newFollowed = await Followed.create({
      userProfileFollowed: idFollowed,
      userNameFollowed: userNameFollowed,
    }); //le insertamos a user2. user1
    const followed = await newFollowed.setUser(findFollowed.idUser);
  };

  unFollowed = async (userNameFollowed, UserIdUser) => {
    const findFollowedInDb = await Followed.findOne({
      //busco a user1
      where: {
        [Op.and]: {
          userNameFollowed: userNameFollowed,
  ...
  getFolloweds = async (req, res) => {
    ...
    try {
      ...
      const followeds = await Followed.findAll({
        where: { UserIdUser: user.idUser },
      });
