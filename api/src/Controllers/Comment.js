...
class CommentClass {
  ...
  postComment = async (req, res) => {
    ...
    try {
      if (!idPost)
        return res.status(400).json({ msgE: "The post was not found" });
      ...
      const createComment = await Comment.create({
        ...
        idUserComment: user.idUser,
        userNameComment: user.userName,
      });
      if (!createComment)
        return res
          .status(400)
          .json({ msgE: "There was an error creating the comment" });
      ...
      return res
        .status(418)
        .json({ msgE: "There was an error trying to post the comment" });
    } catch (error) {
      console.log(error);
      return res
        .status(400)
        .json({ msgE: "There was an error on the request" });
    }
  };
  ...
  deleteComment = async (req, res) => {
    ...
    try {
      ...
      if (!comment)
        return res
          .status(404)
          .json({ msgE: "The comment with that id doest not exist" });
