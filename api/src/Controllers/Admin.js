...
class Admin {
  ...
  getUserStandar = async (req, res) => {
    try {
      ...
      if (!userStandars)
        return res.status(404).json({ msgE: "There aren't standar users" });
  ...
  getUserPremium = async (req, res) => {
    ...
    if (!userPremium)
      return res.status(404).json({ msgE: "There aren't Premium users" });
  ...
  getUserArtist = async (req, res) => {
    ...
    if (!userArtist)
      return res.status(404).json({ msgE: "There aren't Artist users" });
    res.status(200).json(userArtist);
  };
  getPremiumActiveAccounts = async (req, res) => {
    try {
      const accounts = await MembershipUser.findAll({
      ...
      if (accounts.length) {
        ...
        res
          .status(200)
          .json({ msg: "There are no users with Active premium plans" });
      }
    } catch (error) {
  ...
  getPremiumInactiveAccounts = async (req, res) => {
    try {
      ...
      if (accounts.length) {
        ...
        res
          .status(200)
          .json({ msg: "There are no users with inactive premium plans" });
      }
    } catch (error) {
      return res.status(404).json(error.message);
    }
  ...
  getPremiumDebtorAccounts = async (req, res) => {
    try {
      ...
      if (accounts.length) {
        ...
        res
          .status(200)
          .json({ msg: "There are no users with Debtor premium plans" });
      }
    } catch (error) {
      return res.status(404).json(error.message);
    }
  ...
  createPostAdmin = async (req, res) => {
    ...
    try {
      ...
      if (!user)
        return res.status(404).json({ msgE: "Could not find your user" });
      if (user.dataValues.nameTypeUser !== "Admin")
        return res.status(404).json({ msgE: "You are not admin" });
  ...
  updatePostAdmin = async (req, res) => {
    ...
    try {
      ...
      await Post.update(
        {
          contentPost:
            contentPost && contentPost !== postFound.dataValues.contentPost
              ? contentPost
              : postFound.dataValues.contentPost,
          linkContent:
            linkContent && linkContent !== postFound.dataValues.linkContent
              ? linkContent
              : postFound.dataValues.linkContent,
          imagePost:
            imagePost && imagePost !== postFound.dataValues.imagePost
              ? imagePost
              : postFound.dataValues.imagePost,
  ...
  deletePostReport = async (req, res) => {
    try {
      ...
      if (!reportFound)
        return res.status(404).json({ msgE: "Report not found" });
      ...
      const commentPostReport = await Comment.findAll({
        where: { PostIdPost: idPost },
      });
      //Si existen comentarios del post, los borro
      if (commentPostReport.length) {
        await Comment.destroy({ where: { PostIdPost: idPost } });
      }
  ...
  deleteCommentReport = async (req, res) => {
    try {
      ...
      if (!commentFound)
        return res.status(404).json({ msgE: "Comment not found" });
      //Elimino el reporte
      deleteReport = await ReportPostCommentUser.destroy({
        where: { idReport },
        include: {
  ...
  creatPremiumPlan = async (req, res) => {
    let { priceMembership, namePlanPremium, numberOfMonths, discount } =
      req.body;
    try {
      if (discount)
        priceMembership =
          parseFloat(priceMembership) -
          Math.imul(parseFloat(discount), parseFloat(priceMembership)) / 100;
      ...
      if (!existPlan[1]) {
        if (existPlan[0].dataValues.namePlanPremium === namePlanPremium) {
          return res
            .status(400)
            .json({ msgE: "premium plan with that existing name" });
        } else if (
          existPlan[0].dataValues.priceMembership === priceMembership
        ) {
          return res
            .status(400)
            .json({ msgE: "premium plan with that existing price" });
        } else if (existPlan[0].dataValues.numberOfMonths === numberOfMonths) {
          return res
            .status(400)
            .json({ msgE: "premium plan with that number of existing months" });
        }
      } else {
  ...
  editPremiumPlan = async (req, res) => {
    ...
    try {
      if (!idPlanPremium)
        return res.status(404).json({ msgE: "no premium plan id passed" });
      if (discount)
        priceMembership =
          parseFloat(priceMembership) -
          Math.imul(parseFloat(discount), parseFloat(priceMembership)) / 100;
      ...
      if (premiumPlanFound) {
        if (
          premiumPlanFound.dataValues.priceMembership === priceMembership &&
          premiumPlanFound.dataValues.numberOfMonths === numberOfMonths
        ) {
          return res
            .status(400)
            .json({
              msgE: "Premium plan with that price and existing number of months",
            });
        }
      }
      await PlanPremium.update(
        {
          priceMembership:
            ...
            premiumPlanFound.dataValues.priceMembership !== priceMembership
              ? priceMembership
              : premiumPlanFound.dataValues.priceMembership,
          numberOfMonths:
            numberOfMonths &&
            premiumPlanFound.dataValues.numberOfMonths !== numberOfMonths
              ? numberOfMonths
              : premiumPlanFound.dataValues.numberOfMonths,
          discount: discount ? true : false,
        },
        {
          where: { idPlanPremium },
  ...
  cancelPremiumPlan = async (req, res) => {
    ...
    try {
      ...
      return res
        .status(200)
        .json({ msg: "premium plan successfully canceled" });
    } catch (e) {
      console.log(e);
  ...
  getCashFlow = async (req, res) => {
    try {
      ...
      if (!memberships.length)
        return res
          .status(200)
          .json({ msg: "There are no users with membership" });
      let totalAmountOfMoney = 0;
      for (let user = 0; user < memberships.length; user++) {
        totalAmountOfMoney +=
          memberships[user].dataValues.PlanPremium.dataValues.priceMembership;
      }
  ...
  banUser = async (req, res) => {
    ...
    try {
      ...
      if (userFound.dataValues.nameStateUser === "Banned")
        return res.status(400).json({ msgE: "Already banned user" });
      if (userFound.dataValues.nameStateUser === "Inactive")
        return res.status(400).json({ msgE: "User inactive" });
      await User.update(
        { nameStateUser: "Banned", token: null },
        { where: { idUser } }
      );
      await validation.userBanned(
  ...
  unbanUser = async (req, res) => {
    ...
    try {
      ...
      if (userFound.dataValues.nameStateUser !== "Banned")
        return res.status(400).json({ msgE: "User account not banned" });
      await User.update({ nameStateUser: "Active" }, { where: { idUser } });
      await validation.userUnbanned(
        userFound.dataValues.name,
        userFound.dataValues.lastName,
        userFound.dataValues.email
      );
