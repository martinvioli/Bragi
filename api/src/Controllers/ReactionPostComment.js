...
class ReactionPostComment {
  ...
  detectLikePost = async (req, res) => {
    ...
    try {
      ...
      if (!idPost) {
        return res.status(400).json({ msgE: "Post doesn't exist" });
      }
      //Search Data
      const tokenDecode = jwt.decode(token, authConfig.secret);
      const userLikePost = await User.findOne({
        where: { userName: tokenDecode.userName },
      });
      const postFound = await Post.findOne({ where: { idPost } });
      //Validation post-Search
      if (!userLikePost) {
        return res.status(400).json({ msgE: "User not found" });
      }
      if (!postFound) {
        return res.status(400).json({ msgE: "Post not found" });
      }
      //Detect like
      try {
  ...
  likePost = async (req, res) => {
    ...
    try {
      ...
      try {
        ...
        if (likedUser) {
          return res.status(400).json({ msgE: "The post already has a like" });
        }
  ...
  dislikePost = async (req, res) => {
    ...
    try {
      ...
      //Delete Like of post
      try {
        const notExisteLike = await Like.findOne({
          where: {
            [Op.and]: [
              { idUserLikePost: userLikePost.dataValues.idUser },
              { PostIdPost: postFound.dataValues.idPost },
        ...
        if (!notExisteLike)
          return res.status(400).json({ msgE: "Not exist like in the post" });
        await Like.destroy({
          where: {
            [Op.and]: [
              { idUserLikePost: userLikePost.dataValues.idUser },
      ...
      return res
        .status(400)
        .json({ msgE: "Could not get the like of the post" });
    }
  };
